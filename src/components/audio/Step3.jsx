...
export default function Step3({ setStep, suggestedActivity }) {
  ...
  // Fallback colors just in case the API data is slightly delayed
  const c1 = suggestedActivity?.color1 || "#FF3B7C"; // Default Pink
  const c2 = suggestedActivity?.color2 || "#FF8A00"; // Default Orange
  ...
  return (
    <div className="w-full flex flex-col items-center animate-in fade-in zoom-in-95 duration-700 !p-2 sm:!p-4 relative">
      {/* Main Card Container 
        Using the solid dark background (#151419) with a smooth shadow.
        No white borders, no frosted overlays.
      */}
      <div className="relative w-full max-w-sm flex flex-col items-center text-center !p-8 sm:!p-10 z-10">
        {/* Subtle top-right ambient glow using c1 */}
        <div
          className="absolute -top-20 -right-20 w-64 h-64 rounded-full blur-[80px] pointer-events-none"
          style={{ backgroundColor: c1, opacity: 0.1 }}
        />

        {/* --- CONTENT SECTION --- */}

        {/* Small Context Pill (Dark gray, matching the image) */}
        ...
        {/* Dynamic Title 
          Applies your dynamic colors directly to the text itself 
        */}
        <h3
        ...
        <div className="relative w-32 h-32 sm:w-40 sm:h-40 flex-shrink-0 group !mb-8 z-10">
          {/* Dynamic glow based on the passed color */}
          <div
        ...
        <p className="relative z-10 text-[15px] text-gray-300 font-medium leading-relaxed !mb-10 max-w-[280px] !px-2">
          {suggestedActivity?.desc ||
            "Take a moment to engage in this activity to balance your aura."}
        </p>

        {/* --- ACTION BUTTON --- 
          Applies the dynamic gradient to the button background
        */}
        <button
          onClick={() => navigate("/")}
          className="relative z-10 group w-full flex items-center justify-center text-white font-extrabold text-lg rounded-2xl overflow-hidden transition-all duration-300 active:scale-[0.97] !py-4 !px-6"
          style={{
            backgroundImage: `linear-gradient(to right, purple, violet)`,
            // Appends a slight transparency (40 in hex) to the color for a matching shadow
